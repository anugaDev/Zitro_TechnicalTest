...
export class SplashScreenModel implements ISplashScreenModel
{
    ...
    private static readonly FILL_DURATION_MILLISECONDS: number = 500;

    private static readonly TIMER_WEIGHT: number = 0.75;

    private static readonly TARGET_FILL: number = 1

    private static readonly TICK_MILLISECONDS: number = 16;
    ...
    private animateFillToComplete(): void
    {
        ...
        this._fillIntervalId = setInterval(() => this.onFillTick(), SplashScreenModel.TICK_MILLISECONDS);
    }

    private onFillTick(): void
    {
        const elapsed = Date.now() - this._fillStartTime;
        const normalizedProgress = Math.min(elapsed / SplashScreenModel.FILL_DURATION_MILLISECONDS, SplashScreenModel.TARGET_FILL);
